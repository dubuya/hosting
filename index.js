//ODM0MzA4NDMwOTg2MzQ2NTA2.YH_ARg.leJk0-V48Vkfj_HOE6AKUmTamio
...
client.on('message', message => {
  const member = message.mentions.members.first() || message.guild.members.cache.get;
  if (message.content === `${prefix}mute ${member}`) {
    const muteRole = message.guild.roles.cache.find(r = r.name == "Muted");
    if(!muteRole) return message.channel.send("Mute Role Not found. please add role with name `Muted` and try the commands again.")
    member.roles.add(muteRole);
    message.channel.send(`**__${member}__** Muted`)
  }
})
...
client.on('message', message => {
  if (message.content === 'Pagi') {
    ...
    .setTitle(`Pagi juga, ${message.author.username}`)
    .setColor(16438531)
    .setDescription(`Jangan Lupa makan !`);
...
client.on('message', message => {
  if (message.content === 'Siang') {
    ...
    .setTitle(`Siang , ${message.author.username}`)
    .setColor(16426019);
    message.reply(embed);
  }
})
...
client.login('ODM0MzA4NDMwOTg2MzQ2NTA2.YH_ARg.leJk0-V48Vkfj_HOE6AKUmTamio');